...
export function GameDataProvider({ children }: { children: React.ReactNode }) {
  ...
  // Game data state
  const [floors, setFloors] = useState<GameFloor[]>([]);
  const [mythlings, setMythlings] = useState<GameMythling[]>([]);
  const [abilities, setAbilities] = useState<GameAbility[]>([]);
  ...
  const loadGameData = async () => {
    ...
    try {
      ...
      const [floorsData, mythlingsData, abilitiesData] = await Promise.all([
        fetchFloors(),
        fetchMythlings(),
        fetchAbilities(),
      ]);
